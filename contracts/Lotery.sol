...
contract Lotery is Ownable{
  ...
  function finishPlay(uint[] memory randomNumber) public {
   ...
   //Contorlar resultados repetidos
    for(uint i; i < randomNumber.length; i++){
      ...
      priceCoin.transfer( numberOwner[winersNumbers[i]], winAmount(i));
    }
    ...
    for(uint i; i < randomNumber.length; i++){
      ...
      if(i == 0){ 
        ...
        for(uint j; j < i; j++){
          ...
          //recorremos el vector creado en el loop superior, para verificar que no tengamos 
          // winersNumbers repetidos, y que tampoco tengamos address repetidas
  ...
  function referralSystem(address newReferrer) public{
    ...
    if(referralsBuys[RealReferrer] == 3){
